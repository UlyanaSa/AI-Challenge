 ...
 */
@Composable
fun GenerationSettingsForm(
    initial: GenerationSettings,
    applyLabel: String,
    onApply: (GenerationSettings) -> Unit,
    modifier: Modifier = Modifier
) {
    // Черновик формы; применяется только по кнопке с [applyLabel]
    var model by remember(initial) { mutableStateOf(initial.model) }
    var maxTokensText by remember(initial) { mutableStateOf(initial.maxTokens.toString()) }
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            supportingText = {
                Text(
                    "Общий контекст и правила поведения модели. Передаётся с каждым сообщением. " +
                        "Пусто — их задаёт первое сообщение чата."
                )
            },
            minLines = 2
        )
