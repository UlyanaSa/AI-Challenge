 ...
 * факт наличия записи: здесь различаются «правил ещё не задавали» (null — агент работает
 * без правил) и «правил нет» (пустой список — человек убрал все, поэтому проверки
 * конфликта и блока инвариантов в запросе не будет). Набор правил проекта, с которого
 * начинается работа, кладёт сюда сервер при первом запуске, а не агент; поэтому наличие
 * записи значит «правила заданы», и «правил нет» переживает и перезапуск: пустой список
 * так и сохраняется, а не читается обратно как отсутствие записи.
 *
